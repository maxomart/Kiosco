  ...
  const handleEmit = async () => {
    ...
    try {
      ...
      if (!r.ok || !d.ok) {
        ...
        toast.error(d.error ?? "Error al emitir ND")
        return
      }
      setIssued({
        cae: d.cae,
      ...
      <div className="bg-gray-950 border border-gray-800 rounded-2xl w-full max-w-md max-h-[92vh] overflow-y-auto">
        <div className="flex items-center justify-between p-5 border-b border-gray-800">
          <div className="flex items-center gap-2">
            <FilePlus2 size={18} className="text-blue-400" />
            <h2 className="text-lg font-bold text-white">
              {issued ? "Nota de débito emitida" : "Emitir nota de débito"}
            </h2>
          </div>
        ...
        <div className="p-5 space-y-4">
          {issued ? (
            ...
            <>
              <div className="bg-blue-950/30 border border-blue-900/40 rounded-lg p-3 text-xs text-blue-100 flex gap-2">
