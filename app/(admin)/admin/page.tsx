...
import { Plus, Store, Users, Package, ShoppingCart, Check, Copy, Eye, Power, PowerOff, Loader2, X } from "lucide-react"
      ...
      {credentials && (
        <div className="mb-6 bg-green-900/30 border border-green-700 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setCredentials(null)} className="text-gray-400 hover:text-white">
              <X size={18} />
            </button>
          </div>
          ...
          {tenants.map(tenant => (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div
                    className={cn(
                      ...
                      tenant.active ? "bg-blue-600" : "bg-gray-700"
                    )}
                  >
                ...
                >
                  {tenant.active ? <Power size={16} /> : <PowerOff size={16} />}
                </button>
              </div>
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-3xl shadow-2xl w-full max-w-md">
            ...
            <form onSubmit={handleCreate} className="p-6 space-y-4">
              ...
              <p className="text-gray-500 text-xs">
                Se generará automáticamente una contraseña segura de 18 caracteres.
              </p>
              <div className="flex gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setShowCreate(false)}
                  className="flex-1 py-2.5 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded-xl font-semibold transition text-sm"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={creating}
                  className="flex-1 py-2.5 bg-purple-600 hover:bg-purple-700 disabled:opacity-60 text-white rounded-xl font-bold transition text-sm flex items-center justify-center gap-2"
                >
